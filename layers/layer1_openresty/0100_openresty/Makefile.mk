...
$(PREFIX)/bin/resty:
	$(MAKE) --file=../../Makefile.standard PREFIX=$(PREFIX) EXPLICIT_NAME="$(EXPLICIT_NAME)" EXTRACFLAGS="$(SSL_CFLAGS)" EXTRALDFLAGS="$(SSL_LDFLAGS)" OPTIONS=" --with-pcre --without-lua_resty_memcached --without-lua_resty_mysql --without-http_scgi_module --without-http_uwsgi_module --http-fastcgi-temp-path=/tmp --with-http_dav_module --with-http_sub_module --with-http_realip_module --with-http_stub_status_module --with-http_auth_request_module --http-client-body-temp-path=/tmp/nginx_clientbody_temp --http-proxy-temp-path=/tmp/nginx_proxy_temp --with-threads --with-luajit-xcflags='$(SSL_CFLAGS) $(SSL_LDFLAGS)' --with-cc-opt='$(SSL_CFLAGS)' --with-ld-opt='$(SSL_LDFLAGS)' --error-log-path=stderr" download uncompress configure build install
