...
function cache_get()
{
    FILE=${1}
    LIFETIME=${2}
    NEWERS="/etc/hosts /etc/resolv.conf /etc/nsswitch.conf /etc/metwork.config ${MFEXT_HOME}/config/config.ini ${MFMODULE_HOME}/config/config.ini ${MFMODULE_RUNTIME_HOME}/config/config.ini"
